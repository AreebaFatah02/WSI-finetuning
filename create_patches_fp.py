...
def stitching(file_path, wsi_object, downscale = 64):
	start = time.time()
	heatmap = StitchCoords(file_path, wsi_object, downscale=1, bg_color=(0,0,0), alpha=-1, draw_grid=False)
...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir, 
    ...
    for i in range(total):
        ...
        if current_vis_params['vis_level'] < 0:
            if len(WSI_object.level_dim) == 1:
                ...
                wsi = WSI_object.getOpenSlide()
                best_level = wsi.get_best_level_for_downsample(1)  # Change to original resolution
                current_vis_params['vis_level'] = best_level
    ...
    df.to_csv(os.path.join(save_dir, 'process_list_autogen.csv'), index=False)
    print("average segmentation time in s per slide: {}".format(seg_times))
    print("average stiching time in s per slide: {}".format(patch_times))
    return seg_times, patch_times
