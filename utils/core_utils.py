import pdb
# we the VIB team are channing the splitting
...
def train(datasets, cur, args):
    ...
    if args.early_stopping:
        early_stopping = EarlyStopping(patience = 4, stop_epoch=50, verbose=True)
...
def test(datasets, cur, args):
    ...
    return results_dict, test_auc, test_auc, test_f1, test_f1
...
def train_loop_clam(epoch, model, loader, optimizer, n_classes, bag_weight, writer = None, loss_fn = None):
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        logits, Y_prob, Y_hat, Att_scores, instance_dict = model(data, label=label, instance_eval=True,train_epoch=epoch)
